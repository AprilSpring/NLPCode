...
# used for multiple labels predict
flags.DEFINE_integer(
    "labels_num", 18,
    "The numbers of multiple labels")
...
class SelfProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir, labels_num):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "train.tsv")), "train", labels_num)

  def get_dev_examples(self, data_dir, labels_num):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "dev.tsv")), "dev", labels_num)

  def get_test_examples(self, data_dir, labels_num):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "test.tsv")), "test", labels_num)

  def get_labels(self, labels_num):
    """See base class."""
#    return ["0", "1"]
    labels_list = []
...
#    """See base class."""
#    return self._create_examples(
#        self._read_tsv(os.path.join(data_dir, "label.tsv")), "label") 
  ...
  '''
  # 新脚本
  def _create_examples(self, lines, set_type, labels_num):
    """Creates examples for the training and dev sets."""
    examples = []
    for (i, line) in enumerate(lines):
      if i == 0:
...
#        label = tokenization.convert_to_unicode(line[2:]) # train set: id, text, label1, ... , label18
        label = line[2:]
      examples.append(
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  with tf.variable_scope("loss"):
    ...
    # multi-class
##   probabilities = tf.nn.softmax(logits, axis=-1) # *** 输出为值为0-1之间的向量, is multi-class case
...
#    loss = tf.reduce_mean(per_example_loss)
    ...
    # multi-label
    probabilities = tf.nn.sigmoid(logits) # *** is multi-label case
...
def serving_input_fn():
    ...
    label_list = processor.get_labels(FLAGS.labels_num) # *** 样本标签
...
def main(_):
  ...
  if FLAGS.do_train:
    train_examples = processor.get_train_examples(FLAGS.data_dir, FLAGS.labels_num)
    num_train_steps = int(
...
#        drop_remainder=True)
  ...
  if FLAGS.do_eval:
    eval_examples = processor.get_dev_examples(FLAGS.data_dir, FLAGS.labels_num)
  ...
  if FLAGS.do_predict:
    predict_examples = processor.get_test_examples(FLAGS.data_dir, FLAGS.labels_num)
  ...
  if FLAGS.do_export:
    ...
    tf.logging.info("***** Running savedmodel file *****")
