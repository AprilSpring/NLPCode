...
#      self.sequence_output = self.all_encoder_layers[-2] #倒数第二层
...
#    return tf.reduce_mean(self.all_encoder_layers[-2], axis=1)
  ...
  # 修改  
  def get_sentence_encoding(self, input_mask):
    # https://github.com/google-research/bert/issues/71#issuecomment-436507081
    # encoder_layer is [batch_size, max_seq_length, 768]
    # input_mask is [batch_size, max_seq_length]
    input_mask = tf.cast(input_mask, tf.float32)
    mul_mask = lambda x, m: x * tf.expand_dims(m, axis=-1) # is [batch_size, max_seq_length, 1]
    masked_reduce_mean = lambda x, m: tf.reduce_sum(mul_mask(x, m), axis=1) / (
                    tf.reduce_sum(m, axis=1, keepdims=True) + 1e-10)
    encoder_layer = self.all_encoder_layers[-2] # 取倒数第二层
    pooled = masked_reduce_mean(encoder_layer, input_mask) #增加了masked，使得结果仅针对有文本的向量进行reduce_sum操作，去除padding部分的影响
    return pooled

  # 修改  
  def get_sentence_encoding_not_pool(self):
    # encoder_layer is [batch_size, max_seq_length, 768]
    encoder_layer = self.all_encoder_layers[-2] # 取倒数第二层
...
def get_assignment_map_from_checkpoint(tvars, init_checkpoint):
  ...
  # *** get variables
  # *** 返回给定ckpt文件中所有tensor的名称以及对应的shape，返回(name, shape)
...
def embedding_lookup(input_ids,
  ...
  if use_one_hot_embeddings:
    ...
    one_hot_input_ids = tf.one_hot(flat_input_ids, depth=vocab_size) #eg., 20*20000
    output = tf.matmul(one_hot_input_ids, embedding_table) #eg., 20*768
  else:
...
def embedding_postprocessor(input_tensor,
  ...
  if use_token_type:
    ...
    one_hot_ids = tf.one_hot(flat_token_type_ids, depth=token_type_vocab_size) # *** 2*2
    token_type_embeddings = tf.matmul(one_hot_ids, token_type_table) # *** 2*embedding_size
...
# *** 对from_tensor(即input_ids)进行mask
    ...
    with tf.variable_scope("layer_%d" % layer_idx):
      ...
      with tf.variable_scope("attention"):
        ...
        with tf.variable_scope("output"):
          ...
          attention_output = layer_norm(attention_output + layer_input) # *** add residual
