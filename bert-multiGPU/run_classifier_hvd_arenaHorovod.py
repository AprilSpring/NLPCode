...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  with tf.variable_scope("loss"):
    ...
    per_example_loss = -tf.reduce_sum(one_hot_labels * log_probs, axis=-1) # loss = -sum[p0*log(p0) + (p1)*log(p1)], p0+p1=1
...
#  FLAGS.output_dir = FLAGS.output_dir if hvd.rank() == 0 else None #只在worker 0上保存checkpoint,防止checkpoint保存错乱
