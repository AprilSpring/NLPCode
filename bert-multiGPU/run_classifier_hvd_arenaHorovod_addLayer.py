...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  output_weights1 = tf.get_variable(
      ...
      initializer=tf.truncated_normal_initializer(stddev=0.02)) #[hidden_size, hidden_size]

  output_bias1 = tf.get_variable(
      "output_bias1", [hidden_size], initializer=tf.zeros_initializer()) #[hidden_size]
  
  # second
  output_weights2 = tf.get_variable(
      "output_weights2", [num_labels, hidden_size],
      initializer=tf.truncated_normal_initializer(stddev=0.02)) #[num_labels, hidden_size]

  output_bias2 = tf.get_variable(
      "output_bias2", [num_labels], initializer=tf.zeros_initializer()) #[num_labels]
  ...
  with tf.variable_scope("loss"):
    ...
    # first
    logits1 = tf.matmul(output_layer, output_weights1, transpose_b=True) #[batch_size, hidden_size]
    logits1 = tf.nn.bias_add(logits1, output_bias1) # *** 全连接层, [batch_size, hidden_size]
    logits1 = tf.nn.dropout(logits1, keep_prob=0.9) # 添加dropout
    #logits1 = tf.nn.relu(logits1) # 添加激活函数
    logits1 = gelu(logits1) # 添加激活函数
    
    # second
    logits2 = tf.matmul(logits1, output_weights2, transpose_b=True) #[batch_size, num_labels]
    logits2 = tf.nn.bias_add(logits2, output_bias2) # *** 全连接层, #[batch_size, num_labels]
    #logits2 = tf.nn.dropout(logits2, keep_prob=0.9) # 添加dropout
    
    # softmax
    probabilities = tf.nn.softmax(logits2, axis=-1) # *** 输出为值为0-1之间的向量
    log_probs = tf.nn.log_softmax(logits2, axis=-1)
    ...
    return (loss, per_example_loss, logits2, probabilities)
